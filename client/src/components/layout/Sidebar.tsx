...
import { Book, FileText, Settings, FileCode, ChevronLeft, ChevronRight } from "lucide-react";
...
export function Sidebar({ isCollapsed, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <div className={`hidden md:flex md:flex-col ${isCollapsed ? 'md:w-16' : 'md:w-64'} bg-white dark:bg-neutral-800 border-r border-neutral-200 dark:border-neutral-700 h-screen transition-all duration-300 relative`}>
      ...
      <div className="flex items-center h-16 px-4 border-b border-neutral-200 dark:border-neutral-700 overflow-hidden">
        <div className="flex items-center truncate">
          <svg className="h-10 w-10 mr-2 flex-shrink-0 text-primary" viewBox="0 0 24 24">
            <path
              fill="currentColor"
          ...
          {!isCollapsed && <span className="text-xl font-semibold truncate">CodePresenter</span>}
        </div>
      </div>
      ...
      <ScrollArea className="flex-1">
        ...
        {!isCollapsed && recentNotebooks && recentNotebooks.length > 0 && (
            ...
            <div className="mt-2 space-y-1">
              {recentNotebooks.map((notebook: any) => (
                ...
                >
                  ...
                  >
                    <FileCode className="h-4 w-4 mr-3 text-neutral-500 flex-shrink-0" />
